...
326 tests across 12 test files. All tests run without API keys (simulation mode). Typical runtime ~5 seconds.

Test files:
- `src/shared/Goal.test.ts` — 3 tests: priority sorting, prompt generation
- `src/shared/Memory.test.ts` — 4 tests: findings store, summary, copy isolation
- `src/agents/WhtEnvironment.test.ts` — ~96 tests: all tool implementations, parameter validation, source_type/hierarchy (Phase 16)
- `src/agents/BeneficialOwnerAgent.test.ts` — 36 tests: validateInput (Zod), computeReportConfidence, parseFindings
...
- `src/server/EntityRegistry.test.ts` — 48 tests: upsert, audit trail, review status, force-draft (Phases 15+17)
- `src/server/SubstanceInterviewer.test.ts` — 13 tests: state machine logic
- `src/rag/Chunker.test.ts` + `LegalRagService.test.ts` — ~34 tests: frontmatter parsing, source_type, cosine similarity
- `src/rag/Retriever.test.ts` — ~22 tests: similarity, filters, source_type forwarding (Phase 16)
...
| `npm test` | All 326 unit tests (build precondition included) |
| `npm run lint` | ESLint + Prettier check across all TS files |
| `npm run test:coverage` | c8 coverage report (text + lcov) |
| `npm run eval` | Golden dataset evaluation harness — 13 active cases; requires `OPENAI_API_KEY` |
