...
import { TreatyVerifierAgent, TreatyRateVerification, RateVerificationStatus }
...
describe('TreatyVerifierAgent — simulate mode', () => {
  ...
  it('works with royalty income type', async () => {
    const agent  = makeAgent();
    const result = await agent.verifyRate('france', 'royalty', '10%', 'Art. 12(2) Poland–France DTC');
    assertVerificationShape(result);
  ...
  it('works with interest income type', async () => {
    const agent  = makeAgent();
    const result = await agent.verifyRate('germany', 'interest', '5%', 'Art. 11(2) Poland–Germany DTC');
    assertVerificationShape(result);
    assert.equal(result.income_type, 'interest');
  });
  ...
  it('works with dividend income type', async () => {
    const agent  = makeAgent();
    const result = await agent.verifyRate('luxembourg', 'dividend', '5% (reduced, shareholding ≥10%) / 15% (standard)', 'Art. 10(2) Poland–Luxembourg DTC');
    assertVerificationShape(result);
  ...
  it('handles zero rate (0%) without throwing', async () => {
    const agent  = makeAgent();
    const result = await agent.verifyRate('austria', 'interest', '0%', 'Art. 11(2) Poland–Austria DTC (Protocol 2008)');
    assertVerificationShape(result);
